...
class DataExtract(object):
	...
	def getDocumentsWithTerm(self, term):
		...
		for document in self.getDocumentsAsTokens():
			...
			if term in document:
				documentsWithTerm = documentsWithTerm | set(document)
				#appendWith(document)
			else:
				documentsWithoutTerm = documentsWithoutTerm | set(document)
				#appendWithout(document)

		return documentsWithTerm, documentsWithoutTerm

	def getDocumentsAsTokens(self):
		"""	Get collection of documents as DocumentPlus objects as tokens
		...
		for document in self.getDocuments():
			append(document.setOwnTokens())

		return documents

	def getDocuments(self):
		"""	Get collection of documents as DocumentPlus objects
			@return	list of DocumentPlus objects #Strings
		"""
		documents = list()
		append 	  = documents.append
	...
	def getExcerpts(self, term):
		...
		for document in self.getDocuments():
			findings = document.findOwnLines(term)
...
if __name__=="__main__":
	...
	savePath = "C:\\Users\\a5rjqzz\\Desktop\\Python\\files\\20160406_Excerpts_Test.txt"
	...
	DocumentPlus().save(savePath, "\n".join(joined))
	wTerm = de.getDocumentsWithTerm(term[0])
	print ">> Documents with {0}: {1}\t{2}".format(term, len(wTerm), wTerm[:25])
