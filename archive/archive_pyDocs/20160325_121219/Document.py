...
class Document(object):
	...
	def __pad(self, max_left_length = 0, max_right_length = 0):
		"""	[2016/03/24 DEPRECATE THIS] pads both left and right leading and trailing text to match the 
			length of the longest text on either side
			@param  max_left_length: maximum number of spaces to pad left
			@param  max_right_length: maximum number of spaces to pad right  
		"""

		# Loop through each line in the document to find longest line
	...
	def find(self, searchTerm = None, scope = 10, returnFind = False):
		""" [2016/03/24 - DEPRECATE?] searches the text file for the indicated search term and stores it,
	...
	def reset(self):
		...
		self._filePath = None	# Clear string
		self._savePath = None	# Clear string

		self._textFile = list()	# Assign to empty list
		self._dataList = list()	# Assign to empty list

		self._loaded   = False	# Reset state to False (i.e., not loaded)
