...
# 2. [2016/03/11] - changed generators in strip_tags function into one expanded for-loops (for-loop runs ~2.75x faster than prior multiple generators).
# 3. [2016/03/11] - added clear_tabs() and is_blank() methods for use in strip_tags() for-loop.
# - - - - - - - - - - - - -
...
class HTMLStripper(HTMLParser):
	...
	def strip_tags_1(self, fileIn):
		...
		# Make output list to store cleaned data
		output = list()

		# Call append function before loop
		append = output.append

		# Loop through lines in the HTML free data and store non-blank lines
		for line in self.get_data().splitlines():
			if not self.is_blank(line):
				append(self.clear_tabs(line))
