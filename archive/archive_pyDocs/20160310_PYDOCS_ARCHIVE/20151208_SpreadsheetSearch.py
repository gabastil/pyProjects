...
# Version: 1.4 
# (updated: 2015/12/03: added "savePath" variable to save() method)
# (updated: 2015/12/04: optimized processes)
# (updated: 2015/12/07: updated "format" function for loop in prepareTerms() method)
...
class SpreadsheetSearch(SpreadsheetPlus.SpreadsheetPlus, DocumentPlus.DocumentPlus):
    ...
    """
        SpreadsheetSearch() extends SpreadsheetPlus() and DocumentPlus() and expands on them by analyzing their data
        against another spreadsheet containing DROOLS rules.
    """
        ...
        for column in self.spreadsheet:
            if lower(column[0]) == lower(name):
                for cell in range(len(column[1:])):
                    column[cell+1] = replace(fillWith, "{C}", str(cell+2))
