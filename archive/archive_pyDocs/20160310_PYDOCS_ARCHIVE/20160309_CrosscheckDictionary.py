...
class CrosscheckDictionary(object):
	...
	def open(self, spreadsheetFile = None, dictionaryFile = None, spreadsheetFileTermsToCheck = 0, dictionaryFileTerms = 0):
		...
		#load   = (s.load() for s in files)
		#init   = (s.initialize() for s in files)
	...
	def check(self, spreadsheet, columnToBeChecked, columnFromDictionary):
		...
		#print columnToBeChecked

		for row in columnToBeChecked:

			if row.isspace():
				spreadsheet.spreadsheet[index] += [self.formulaEmptyCell]
				#print "Index ", index, row, "is space", row.isspace()
			else:
				if row in columnFromDictionary:
					#print spreadsheet.spreadsheet[index][-2], row, columnFromDictionary[columnFromDictionary.index(row)],columnFromDictionary.index(row)
					#print "Index ", index
					spreadsheet.spreadsheet[index] += [self.formulaInDict]
				else:
					#print "Index: {0}\trow: |{1}|\trow in dictionary: {2}	 \trow from dictionary: |".format(index, row, row in columnFromDictionary)
