...
class DICESearch(Compiler, SpreadsheetSearch):
	...
	"""
	1.) findAll()				output goes to (2.) uses findInstance to find individual words in text.
	...
	def buildDictionary(self, keyword, assignment, dictionary):
		"""
			buildDictionary() creates an dictionary with keywords and assignment (S or I) definitions.
			Returns a completely built dictionary with nested dictionaries where applicable.

			keyword:	list of keywords to be defined.
			assignment:	defintion of keyword.
			dictionary:	dictionary to be built.
		"""
		if len(keyword) == 1:
			...
			buildDictionary = self.buildDictionary
			# - - - - - - - - - - - - - - - - - -

			for word in keyword:
				return buildDictionary(keyword[1:], assignment, dictionary[word])
		...
		for c in xrange(len(self.spreadsheet)):		# Iterate through the spreadsheet's columns to search for the "kernel"
			...
			if lower(column[0]) == "kernel":		# Examines to see if the column contains the kernel
				...
				for r in range(len(column))[1:]:	# Iterate through the column to create a string from the left kernel, center kernel, and right kernel columns
					...
					append(leftColumn[r][1:-1] + column[r] + rightColumn[r][1:-1])	# Append string to kernel list
	...
	def getType(self, terms):
		...
		COMMENTED OUT
		listOfTypes = list()
		for i in xrange(len(self.toTYPE)):
	...
	def isSufficient(self, dbPath = "L:\DICE Documents\JoshsAwesomeScript\search-keywords-all.txt"):
		...
		print "[3: KEYWORDS]\tKeywords building ..."
		startClock = clock()
		keywords = ((split(line,'\t')[0], split(line,'\t')[1]) for line in split(keywords,'\n') if len(line) > 1)
		print "[3: KEYWORDS]\tKeywords building complete. Duration : {0}".format(clock() - startClock)

		print "\n[4: DB MATCHING]Checking DB for matches ..."
		startClock1 = clock()
		print "[4: DB MATCHING]Compiling dbCheckResultsList ..."
		dbCheckResultsList = ((d[2:5], d[-1], index(x)+1, x) for d, v in keywords for x in kernel if lower(v) in x)
		print "[4: DB MATCHING]Compiling Complete."
		print "[4: DB MATCHING]Updating spreadsheet ..."

		counter = 0

		# d = dice code, s =  sufficiency, i = excerpt index, x = excerpt/variant
		for d, s, i, x in dbCheckResultsList:
			if d == self.spreadsheet[1][i] and s == 'S':
		...
		print "[#] Complete ({0} found)".format(counter)

		print "\n[4: DB MATCHING]\tUpdating spreadsheet complete. Duration : {0}".format(clock() - startClock1)
