...
class DICESearch(Compiler, SpreadsheetSearch):
	...
	def getExcerpts(self, text, DICECodeResults):
		"""
			...
			getKernels()		input: tuple of indices						output: string
			getComboTerms()		input: tuple of search terms and indices	output: string
			getProximity()		input: tuple of search terms and indices	output: absolute integer
		...
		for DICECode, variant in keywords:
			for excerpt in kernel:
				if variant.lower() in excerpt.lower():
					...
					if int(DICECode[2:5]) == self.spreadsheet[1][i]:

						print "[4: DB MATCHING]\t\tMatch found for {0}".format(i)

						if DICECode[-1].lower() == 's':
