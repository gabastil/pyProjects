...
# 
# Updates:	
# 1. [2015/12/03] - changed list in isSufficient() to generator
# 2. [2015/12/04] - added a load bar to isSufficient() method
# 3. [2015/12/07] - changed getType() to return a list comprehension
# 4. [2015/12/09] - updated the progress bar in isSufficient()
# 5. [2015/12/08] - removed formatting for left and right kernels (i.e., '|' separator). Commented out buildDictionary() method. Make getProximity() sub-method return a non-absolute value.
# 6. [2015/12/14] - changed str to int in line 437 in isSufficient() method in 'keywords' generator. Removed progress bar portion of code.
# 7. [2016/02/29] - changed wording of notes in line 17 from '... class is used in the following ...' to '... class is directly inherited by the following ...'. Version changed from 1.4 to 1.4.1.
# - - - - - - - - - - - - -
"""loads and compiles vocabulary from types.gd for CAPD-/DICE-Tuning documents from client sites.

This finds all the words (indicated by #W00) defined by type.gd in the document input. These excerpts are then assigned to types (indicated by #ABC)
"""
...
class DICESearch(Compiler, SpreadsheetSearch):
	"""	DICESearch order of operations:
		...
		1.) findAll()			find words in text
		2.) getTypesAndIndices()	
		3.) getDICECode()			
		4.) getExcerpts()		after attaching document name to each row
		5.) reviseSpreadsheet()	after revising the spreadsheet and blocking doubles
	...
	def __init__(self, scope = 100):
		"""	initialize the DICESearch class. 
			Pre-compile data structures and mappings from types.gd with 
			self.compileTypes(). Initialize spreadsheetSearch with 
			super(DICESearch, self).__init__(). Construct columns for spread-
			sheet.

			scope --> character range within which to search for combos
		"""
	...
	def buildKernel(self):
		"""	creates a new data structure containing strings combined from the 
			left kernel, center kernel, and right kernel of the spreadsheet. 
			This method requires there be data in the self.spreadsheet vari-
			able in order to return anything. This method returns a list of 
			strings.
	...
	def buildSpreadsheet(self, getTempBuild = False):
		"""	buildSpreadsheet() creates a string from the data structure in self.
			spreadsheet in preparation for writing to a text file. This method 
			returns a temporary or final build. If a temporary build is indic-
			ated, the data structure takes on the following form: 

			["first\tcombined\trow", "second\tcombined\trow"]

			getTempBuild --> boolean that tells that method whether or not to 
							 return a string, or a list of all the rows as 
							 strings.
	...
	def findAll(self, text):
		"""	searches for all the terms as indicated in the types.gd file. This 
			method returns a list of tuples, which consist of two parts (index 
			number, term indices list). The index number refers to the word-to-
			terms mapping index number. The term indices refer to the list con-
			taining the terms' locations in the document.

			text --> string in which the term is to be found.
	...
	def findInstance(self, text, term):
		"""
			findInstance() searchs for a term in a text and records the init-
			ial and ending indices of that term if found in the text. This 
			method returns a list of indices in a list where every two numb-
			ers corresponds to the initial and ending indices of the search 
			term respectively, e.g., if term="the" were found 3 times in a 
			text, it could return [3, 6, 10, 13, 23, 26].

			text --> string in which the term is to be found.
			term --> string keyword to search in text.
	...
	def getExcerpts(self, text, DICECodeResults):
		"""	takes the indices provided by findAll(), getTypesAndIndices(), and 
			getDICECode() to get the appropriate strings from the text document.
			This method contains three sub-methods that create the excerpts, 
			list the combo terms, and calculate the proximity of the combo terms
			within the combined kernels. This method returns a list that expands 
			on the data structure DICECodeResults.

			text            --> string from which to extract the text as a string.
			DICECodeResults --> results from getDICECode() used to further build the data structure.

			Uses the following sub-methods:

				getKernels(indices)	  in: tuple of indices			 out: string
				getComboTerms(tuples) in: tuple of terms and indices out: string
				getProximity(tuples)  in: tuple of terms and indices out: +int
	...
	def getDICECode(self, typesAndIndicesResults):
		"""	getDICECode() appends the DICE Code associated with the type in the
			typesAndIndicesResults variable, which comes from the 
			getTypesAndIndices() method. This method returns a list of tuples 
			having the following format:

				[(DICE Code, TYPE Code, [Indices of Matched TERMS])]

			typesAndIndicesResults --> a list of types and indices of matched 
									   terms. Outputted by getTypesAndIndices()
									   method.
									
									   This variable has the following struct-
									   ure:
	...
	def getTypesAndIndices(self, findAllResults = None):
		"""	getTypesAndIndices() iterates through the results of findAll() and
			checks to see if they are within range of each other for TYPES 
			that contain TERMS with more than one WORD (e.g., [W01, W02, etc.]).
			This method returns a list of tuples taking on the following struct-
			ure: [([TYPE], [Indices of Matched Terms])].

			findAllResults --> list of tuples outputted by the findAll() method. 
			Takes on the following structure: ([TYPE], [Indices of Matched TERMS])
	...
	def isSufficient(self, dbPath = "L:\DICE Documents\JoshsAwesomeScript\search-keywords-all.txt"):
		"""	isSufficient() compares terms in the search-keywords-all.txt file,
			which contains relevant variants from the database, to the terms
			found in self.spreadsheet. A new kernel is built from the left 
			kernel, center kernel, and right kernel from the self.spreadsheet
			data structure.

			dbPath --> Location of the database file to check the built kernel 
				       against.
		...
		startClock 	= clock()
		keywords 	= ((int(split(line,'\t')[0][2:5]), lower(split(line,'\t')[1])) for line in split(keywords,'\n') if len(line) > 1 and split(line, '\t')[0][-1] == 'S')
		print "Complete ({0} seconds)".format(round(clock() - startClock, 2))

		print "[2C: MATCHES]\tCompiling results ...",
		startClock1 		= clock()
		dbCheckResultsList 	= ((index(k)+1, v) for d, v in keywords for k in kernel if v in k if self.spreadsheet[1][index(k)+1] == d)
		print "Complete ({0} seconds)".format(round(clock() - startClock, 2))
		...
		for i, v in dbCheckResultsList:
			self.spreadsheet[-2][i] = 1
			self.spreadsheet[-1][i] = v

			counter += 1

		print "Complete ({0} sufficiencies found)".format(counter)
	...
	def isTermInRange(self, findAllResults = None, terms = None):
		"""	isTermInRange() checks to see if the terms within the findAllResults
			variable are close enough to each other if there are multiple terms.
			If they are single terms, then the term is automatically indicated 
			as being in range (i.e., return True). This method returns a tuple 
			containing a boolean value indicating the term's status as in-range 
			or out-of-range as well as a list of indices for the terms that fall
			within range of each other.

			findAllResults --> list of tuples outputted by the findAll() method. 
							   Takes on the following structure: ([TYPE], 
							   [Indices of Matched TERMS])
			terms 		   --> list containing terms (e.g., W01, W02, etc.) that 
							   need to fall within a certain range of each other 
							   with the structure: [W01, W03, etc.]
	...
	def reviseSpreadsheet(self, getExcerptsResults, sourceDocumentName, sheet = 1):
		"""	reviseSpreadsheet() adds rows from getExcerptsResults as well as the 
			document name in sourcDocumentName to the current spreadsheet.

			getExcerptsResults --> results from the getExcerpts() method
			sourceDocumentName --> name of the document analyzed with 
					               getExcerpts(). Typically following the format
					               *.txt.
