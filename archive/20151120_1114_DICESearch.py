	...
	def getExcerpts(self, text, DICECodeResults):
		...
		for row in DICECodeResults:
			...
			if type(row[2][0]) == type(tuple()) and len(row[2]) > 1:
				...
				kernel 		= repr(text[i:j]).replace("\\n", "_")
				kernelLeft 	= repr(text[h:i]).replace("\\n", "_")
				kernelRight = repr(text[j:k]).replace("\\n", "_")

				"""
				left = repr(text[row[2][0]-self.scope:row[2][0]])
				right = repr(text[row[2][1]:row[2][1]+self.scope])

				print "kernel:\t{0}".format(kernel)
				print repr("left: {0}".format(left).replace("\\n", "_"))	
				print repr("right: {0}".format(right).replace("\\n", "_"))				
				#print "NO TUPLES", kernel, row[2][0], row[2][1], row[2]
				"""

				print kernelLeft, kernel, kernelRight
			print DICECode, TYPECode
