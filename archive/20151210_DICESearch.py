...
class DICESearch(Compiler, SpreadsheetSearch):
	...
	def isSufficient(self, dbPath = "L:\DICE Documents\JoshsAwesomeScript\search-keywords-all.txt"):
		...
		dbCheckResultsList 	= ((index(x)+1, x) for d, v in keywords for x in kernel if lower(v) in x if self.spreadsheet[index(x)+1] == str(int(d[2:5])))
		print "Complete ({0} seconds)".format(round(clock() - startClock, 2))

		print "[2D: UPDATES]\tUpdating document ..."

		#print len(list(dbCheckResultsList))

		counter = 0.
		#ss1 = self.spreadsheet[1]
		#ss2 = self.spreadsheet[-2]
		#ss3 = self.spreadsheet[-1]
